...
#define _XM_FILE_NAME analyze_c
...
static void analyze_note_trigger(xm_context_t* ctx, xm_channel_context_t* ch, uint64_t* used_features) {
    ...
    /* Use current->note, NOT orig_note, this matters for tone
       portamentos */
    note = (int16_t)(ch->current->note + RELATIVE_NOTE(smp));
    if (note <= 0 || note >= 120) {
...
void xm_analyze(xm_context_t* ctx, char* out) {
    ...
    xm_channel_context_t* ch;
        ...
        #if defined(_XM_CMAKE) && _XM_CMAKE == 1
            puts("xm_analyze: suggested flags will be inaccurate; recompile libxm with"
            ...
            puts("xm_analyze: suggested flags will be inaccurate; recompile libxm with \n"
                 ...
                 "#define XM_LOOPING_TYPE 2\n"
                 "in xm.h to suppress this warning");
        #endif
    #endif
 ...
 break2:
    ...
    while(XM_LOOPING_TYPE != 0 && LOOP_COUNT(ctx) == 0) {
        ...
        ch = ctx->channels;
        for(i = 0; i < NUM_CHANNELS(&ctx->module); ++i, ++ch) {
            /* XXX */
            assert(ch->current->effect_type < 64 || ch->current->effect_type == EFFECT_NOP);
            _XM_STATIC_ASSERT_FUNCTION(EFFECT_ARPEGGIO == 0, "");
            if(ch->current->effect_type == 0) {
                ...
                /* XXX: some effects can be checked after the
                   volume==0 check */
                used_effects |= (uint64_t)1 << ch->current->effect_type;
            }
            used_volume_effects |= (uint16_t)1 << (VOLUME_COLUMN(ch->current) >> 4);
            ...
            if(ch->instrument->panning_envelope.num_points) {
                used_features |= (uint64_t)1 << FEATURE_PANNING_ENVELOPES;
            }
            #endif

            #if HAS_FEATURE(FEATURE_AUTOVIBRATO)
            if(ch->instrument->vibrato_depth && (ch->instrument->vibrato_rate > 0 || ch->instrument->vibrato_type == WAVEFORM_SQUARE)) {
                /* A zero vibrato_rate effectively turns off
                   autovibrato, except for square waveforms */
                used_features |= (uint64_t)1 << FEATURE_AUTOVIBRATO;
                used_features |= (uint64_t)1 << FEATURE_WAVEFORM(ch->instrument->vibrato_type);
            }
            #endif

            panning = (int16_t) (xm_get_panning_of_channel(ctx, i+1) * (float)MAX_PANNING + 0.5f);
            if(pannings[i % 4] == -1) {
    ...
    if(tempo > 0) {
        ...
        used_features |= (uint64_t)((~tempo) & 31) << FEATURE_VARIABLE_TEMPO;
        used_effects &= ~((uint64_t)1 << EFFECT_SET_TEMPO);
    } else {
        _XM_STATIC_ASSERT_FUNCTION(MIN_BPM == 32,"");
        used_features |= (uint64_t)31 << FEATURE_VARIABLE_TEMPO;
    }

    if(bpm > 0) {
        assert(bpm >= MIN_BPM && bpm <= MAX_BPM);
        used_features |= (uint64_t)((~bpm) & 255) << FEATURE_VARIABLE_BPM;
        used_effects &= ~((uint64_t)1 << EFFECT_SET_BPM);
    } else {
        _XM_STATIC_ASSERT_FUNCTION(MAX_BPM == 255,"");
        used_features |= (uint64_t)255 << FEATURE_VARIABLE_BPM;
    }

    _XM_STATIC_ASSERT_FUNCTION(MAX_CHANNELS == 255,"");
    used_features |= (uint64_t)((~NUM_CHANNELS(&ctx->module)) & 255) << FEATURE_VARIABLE_CHANNEL_COUNT;
    ...
    #else
        append_str(out, &off, "\n#define XM_DISABLED_EFFECTS 0x");
        append_u64(out, &off, (uint64_t)(~used_effects));

        append_str(out, &off, "\n#define XM_DISABLED_VOLUME_EFFECTS 0x"); 
        append_u16(out, &off, (uint16_t)(~used_volume_effects)); 

        append_str(out, &off, "\n#define XM_DISABLED_FEATURES 0x"); 
        append_u64(out, &off, (uint64_t)(~used_features)); 

        append_str(out, &off, "\n#define XM_PANNING_TYPE ");
        append_char(out, &off, (char)('0' + panning_type));
        append_str(out, &off, "\n\n"); 
    #endif
