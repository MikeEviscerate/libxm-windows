...
/* libxm contributor: Dan Spencer <dan@atomicpotato.net> */
...
#define _XM_FILE_NAME load_c



#define EMPTY_PATTERN_NUM_ROWS 64
#define SAMPLE_HEADER_SIZE 40
#define SAMPLE_FLAG_16B 0x10
#define SAMPLE_FLAG_PING_PONG 0x2
#define SAMPLE_FLAG_FORWARD 0x1
#define ENVELOPE_FLAG_ENABLED 0x1
#define ENVELOPE_FLAG_SUSTAIN 0x2
#define ENVELOPE_FLAG_LOOP 0x4
...
#define READ_MEMCPY_BOUND(dest, offset, length, bound) \
    memcpy(dest, (uint8_t*)(moddata) + (offset), \
                     ((offset) + (length) <= (bound)) ? \
                     (length) : ((offset) >= (bound) ? \
                                 0 : (bound) - (offset)))
...
#if _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int16_t 
    #define SAMPLE_POINT_FROM_S8(v) ((v) * 256)
    #define SAMPLE_POINT_FROM_S16(v) (v)
    #define SAMPLE_POINT_FROM_F32(v) ((int16_t)((v) * 32768.f))

#elif _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int8_t 
    #define SAMPLE_POINT_FROM_S8(v) (v)
    #define SAMPLE_POINT_FROM_S16(v) xm_dither_16b_8b(v)
    #define SAMPLE_POINT_FROM_F32(v) xm_dither_16b_8b((int16_t)((v) * 32768.f))

#elif _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_float 
    #define SAMPLE_POINT_FROM_S8(v) ((float)(v) / 128.f)
    #define SAMPLE_POINT_FROM_S16(v) ((float)(v) / 32768.f)
    #define SAMPLE_POINT_FROM_F32(v) (v)
...
struct xm_prescan_data_s {
    ...
    _XM_STATIC_ASSERT_STRUCT(MAX_PATTERNS* MAX_ROWS_PER_PATTERN <= 0xFFFFFF, "")
    uint8_t format;
...
bool xm_prescan_module(const char* moddata, uint32_t moddata_length, xm_prescan_data_t* out) {
    ...
    NOTICE("xm_prescan_module", "input data does not look like a supported module");
    return false;

 end:
    #if HAS_HARDCODED_CHANNEL_COUNT
    if(out->num_channels != HAS_HARDCODED_CHANNEL_COUNT) {
        NOTICE("xm_prescan_module", "unsupported number of channels (expected %u, has %u)",
            (unsigned int)HAS_HARDCODED_CHANNEL_COUNT, out->num_channels);
        return false;
    }
    #endif

    sz = sizeof(xm_context_t);
    if(_xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_pattern_t) * out->num_patterns)
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_pattern_slot_t) * out->num_rows * out->num_channels)
       #if HAS_INSTRUMENTS
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_instrument_t) * out->num_instruments)
       #endif
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_sample_t) * out->num_samples)
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_sample_point_t) * out->samples_data_length)
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(xm_channel_context_t) * out->num_channels)
       #if XM_LOOPING_TYPE == 2
       || _xm_ckd_add_u32_uu(&sz, sz, sizeof(uint8_t) * MAX_ROWS_PER_PATTERN * out->pot_length)
       #endif
       ) {
        NOTICE("xm_prescan_module", "module too big for uint32");
        return false;
    }
    if(sz > 128 << 20) {
    ...
    NOTICE("xm_prescan_module", "read %d patterns, %d channels, %d rows, %d instruments, %d samples, %d sample frames, %d pot length",
...
xm_context_t* xm_create_context(char* mempool,
...
static void xm_fixup_common(xm_context_t* ctx) {
    ...
    for(i = ctx->module.num_rows * NUM_CHANNELS(&ctx->module); i; --i, ++slot) {
...
void xm_dump_context(xm_context_t* ctx, char* out) {
    uint32_t i;
    uint32_t ctx_size;
    uint64_t old_hash;
    ...
    /* (*) Everything done after this should be deterministically
       reversible */
    ctx_size = xm_dump_size(ctx);
    old_hash = xm_fnv1a((void*)ctx, ctx_size);

    /* Do nothing for floats, in practice this doesn't help */
    #if XM_LIBXM_DELTA_SAMPLES && (_XM_typeof(XM_SAMPLE_TYPE) != _XM_type_float)
    for(i = ctx->module.samples_data_length - 1; i > 0; --i) {
...
xm_context_t* xm_restore_context(char* data) {
    ...
    #if XM_LIBXM_DELTA_SAMPLES && (_XM_typeof(XM_SAMPLE_TYPE) != _XM_type_float)
    for(i = 1; i < ctx->module.samples_data_length; ++i) {
        ctx->samples_data[i] += ctx->samples_data[i-1];
    }
    #endif
...
static bool xm_prescan_xmif(const char* moddata,
                            uint32_t moddata_length,
                            xm_prescan_data_t* out) {
    uint32_t num_rows;

    if(READ_U8(1) > 0) {
        NOTICE("xm_prescan_xmif", "version too new");
        return false;
    }

    num_rows = READ_U32(0x0C);
    if(num_rows >= 0x1000000) {
        NOTICE("xm_prescan_xmif", "too many rows");
        return false;
    }
...
static void xm_load_xmif(xm_context_t* ctx,
    ...
    for(i = 0; i < ctx->module.num_instruments; ++i) {
        ...
        envs[1] = &(ctx->instruments[i].panning_envelope);
        #else
        envs[1] = 0;
        #endif
        ...
        for(j = 0; j < 2; ++j) {
            ...
            _XM_STATIC_ASSERT_FUNCTION(MAX_ENVELOPE_POINTS >= 12, "");
            for(k = 0; k < 12; ++k) {
                envs[j]->points[k].frame = READ_U16(offset + 0x34*j + 0x04*k);
                envs[j]->points[k].value = (uint8_t)READ_U16(offset + 0x34*j + 0x04*k + 0x02);
            }
            envs[j]->num_points = READ_U8(offset + 0x34*j + 0x30);
            envs[j]->sustain_point = READ_U8(offset + 0x34*j + 0x31);
            envs[j]->loop_start_point = READ_U8(offset + 0x34*j + 0x32);
            envs[j]->loop_end_point = READ_U8(offset + 0x34*j + 0x33);
        }
        ...
        _XM_STATIC_ASSERT_FUNCTION(MAX_NOTE == 96, "");
        for(j = 0; j < MAX_NOTE; ++j) {
            ctx->instruments[i].sample_of_notes[j] = READ_U16(offset + 0x70 + 2*j);
        }
        #endif
    ...
    for(i = 0; i < ctx->module.samples_data_length; ++i) {
        ctx->samples_data[i] = SAMPLE_POINT_FROM_F32(U32_TO_F32(READ_U32(offset)));
        offset += 4;
    }

    for(i = 0; i < PATTERN_ORDER_TABLE_LENGTH && i < ctx->module.length; ++i) {
...
_XM_STATIC_ASSERT_GLOBAL(XMIF_HEADER_SZ % 8 == 0
              && XMIF_PATTERN_SZ % 8 == 0
...
uint32_t xm_save_size(const xm_context_t* ctx) {
    uint32_t sz = 0, slots_sz = 0, samples_sz = 0;
    assert(!_xm_ckd_add_u32_uu(&sz, XMIF_HEADER_SZ, ctx->module.num_patterns * XMIF_PATTERN_SZ));
    assert(!_xm_ckd_mul_u32_uu(&slots_sz, ctx->module.num_rows, XMIF_PATTERN_SLOT_SZ * NUM_CHANNELS(&ctx->module)));
    assert(!_xm_ckd_add_u32_uu(&sz, sz, slots_sz));
    assert(!_xm_ckd_add_u32_uu(&sz, sz, NUM_INSTRUMENTS(&ctx->module) * XMIF_INSTRUMENT_SZ));
    assert(!_xm_ckd_add_u32_uu(&sz, sz, ctx->module.num_samples * XMIF_SAMPLE_SZ));
    assert(!_xm_ckd_mul_u32_uu(&samples_sz, ctx->module.samples_data_length, 4));
    assert(!_xm_ckd_add_u32_uu(&sz, sz, samples_sz));
    assert(!_xm_ckd_add_u32_uu(&sz, sz, ctx->module.length * 2));
    return sz;
}
...
void xm_save_context(const xm_context_t* ctx, char* out) {
    ...
    memcpy(out + 0x02, "LIBXMIF\xFF", 8); /* magic */
    ...
    for(i = 0; i < NUM_CHANNELS(&ctx->module); ++i) {
        WRITE_U8(out + 0x40 + i, DEFAULT_CHANNEL_PANNING(&ctx->module, i));
    }
    out += XMIF_HEADER_SZ;
    ...
    for(i = 0; i < NUM_INSTRUMENTS(&ctx->module); ++i) {
        ...
        for(j = 0; j < 2; ++j) {
            ...
            _XM_STATIC_ASSERT_FUNCTION(MAX_ENVELOPE_POINTS >= 12, "");
            for(k = 0; k < 12; ++k) {
                WRITE_U16(out + 0x34*j + 0x04*k, envs[j]->points[k].frame);
                WRITE_U16(out + 0x34*j + 0x04*k + 0x02, envs[j]->points[k].value);
            }
...
static bool xm_prescan_xm0104(const char* moddata,
    ...
    if(num_channels > MAX_CHANNELS) {
        NOTICE("xm_prescan_xm0104", "module has too many channels (%u > %u)", num_channels, MAX_CHANNELS);
        return false;
    }
    out->num_channels = (uint8_t)num_channels;
    out->num_patterns = READ_U16(offset + 10);
    if(out->num_patterns > MAX_PATTERNS) {
        NOTICE("xm_prescan_xm0104", "module has too many patterns (%u > %u)", out->num_patterns, MAX_PATTERNS);
        return false;
    }
    num_instruments = READ_U16(offset + 12);
    if(num_instruments > MAX_INSTRUMENTS) {
        NOTICE("xm_prescan_xm0104", "module has too many instruments (%u > %u)", num_instruments, MAX_INSTRUMENTS);
        return false;
    }
    _XM_STATIC_ASSERT_FUNCTION(MAX_INSTRUMENTS <= UINT8_MAX,"");
    ...
    for(i = 0; i < out->num_patterns; ++i) {
        num_rows = READ_U16(offset + 5);
        packed_size = READ_U16(offset + 7);
        if(packed_size == 0 && num_rows != EMPTY_PATTERN_NUM_ROWS) {
            NOTICE("xm_prescan_xm0104", "empty pattern %x has incorrect number of rows, overriding (%u -> %u)", i, num_rows, EMPTY_PATTERN_NUM_ROWS);
            num_rows = EMPTY_PATTERN_NUM_ROWS;
        }

        if(num_rows > MAX_ROWS_PER_PATTERN) {
            NOTICE("xm_prescan_xm0104", "pattern %x has too many rows (%u > %u)", i, num_rows, MAX_ROWS_PER_PATTERN);
            return false;
        }
    ...
    for(i = 0; i < out->pot_length; ++i) {
        if(pot[i] >= out->num_patterns) {
            if(out->num_patterns >= MAX_PATTERNS) {
                NOTICE("xm_prescan_xm0104", "no room left for blank pattern to replace an invalid pattern");
                return false;
            }

            NOTICE("xm_prescan_xm0104", "replacing invalid pattern %d in pattern order table with empty pattern", pot[i]);
    ...
    for(i = 0; i < out->num_instruments; ++i) {
        ...
        for(j = 0; j < num_samples; ++j) {
            ...
            if(flags & SAMPLE_FLAG_16B) {
                /* 16-bit sample data */
                if(sample_length % 2) {
                    NOTICE("xm_prescan_xm0104", "sample %d of instrument %d is 16-bit with an odd length!", j, i+1);
                }
                sample_length /= 2;
            }
            ...
            if(HAS_FEATURE(FEATURE_MULTISAMPLE_INSTRUMENTS) || j == 0) {
...
static uint32_t xm_load_xm0104_module_header(xm_context_t* ctx,
    ...
    /* Read XM header */
    #if XM_STRINGS
    _XM_STATIC_ASSERT_FUNCTION(MODULE_NAME_LENGTH >= 21,""); /* +1 for NUL */
    _XM_STATIC_ASSERT_FUNCTION(TRACKER_NAME_LENGTH >= 21,"");
    ...
    if(mod->length > PATTERN_ORDER_TABLE_LENGTH) {
        NOTICE("xm_load_xm0104_module_header", "clamping module pot length %d to %d\n", mod->length, PATTERN_ORDER_TABLE_LENGTH);
        mod->length = PATTERN_ORDER_TABLE_LENGTH;
    }

    #if XM_LOOPING_TYPE != 1
    restart_position = READ_U16(offset + 6);
    if(restart_position >= mod->length) {
        NOTICE("xm_load_xm0104_module_header", "zeroing invalid restart position (%u -> 0)", restart_position);
        restart_position = 0;
    }
    _XM_STATIC_ASSERT_FUNCTION(UINT8_MAX >= PATTERN_ORDER_TABLE_LENGTH - 1,"");
    mod->restart_position = (uint8_t)restart_position;
    #endif

    #if !HAS_HARDCODED_CHANNEL_COUNT
    /* Prescan already checked MAX_CHANNELS */
    _XM_STATIC_ASSERT_FUNCTION(MAX_CHANNELS <= UINT8_MAX,"");
    ...
    *out_num_instruments = READ_U8(offset + 12);
    ...
    if(flags & 0xFE) {
        NOTICE("xm_load_xm0104_module_header", "unknown flags set in module header (%d)", flags);
    }

    #if !HAS_HARDCODED_TEMPO
    tempo = READ_U16(offset + 16);
    if(tempo >= MIN_BPM) {
        NOTICE("xm_load_xm0104_module_header", "clamping tempo (%u -> %u)", tempo, MIN_BPM-1);
        tempo = MIN_BPM-1;
    }
    ...
    #if !HAS_HARDCODED_BPM
    bpm = READ_U16(offset + 18);
    if(bpm > MAX_BPM) {
        NOTICE("xm_load_xm0104_module_header", "clamping bpm (%u -> %u)", bpm, MAX_BPM);
        bpm = MAX_BPM;
    }
...
static uint32_t xm_load_xm0104_pattern(xm_context_t* ctx,
    ...
    slots = ctx->pattern_slots + pat->rows_index * NUM_CHANNELS(&ctx->module);

    packing_type = READ_U8(offset + 4);
    if(packing_type != 0) {
        NOTICE("xm_load_xm0104_pattern", "unknown packing type %d in pattern", packing_type);
    }
    ...
    for(j = 0, k = 0; j < packed_patterndata_size; ++k) {
        ...
        if(slot->effect_type > 0xF) {
            ...
            default:
                ;
            blank_effect:
                NOTICE("xm_load_xm0104_pattern", "deleting effect %02X%02X", slot->effect_type, slot->effect_param);
                slot->effect_type = 0;
                slot->effect_param = 0;
                break;
            case 16:
            ...
            if((slot->effect_type == EFFECT_SET_VIBRATO_CONTROL
               ...
               && (slot->effect_param & 127) == WAVEFORM_RANDOM) {
                ...
                slot->effect_param = (slot->effect_param & 128) | WAVEFORM_SQUARE;
            }
        }
    }
...
static uint32_t xm_load_xm0104_instrument(xm_context_t* ctx,
    ...
    uint32_t ins_header_size, orig_moddata_length, extra_samples_size;
    uint16_t samples_index, i, num_samples;
    uint8_t type, j;
    ...
    ins_header_size = READ_U32(offset);
    /* Original FT2 would load instruments with a direct read into the
           ...
           size. */
    orig_moddata_length = moddata_length;
    moddata_length = offset + ins_header_size;

    type = READ_U8(offset + 26);
    if(type != 0) {
        NOTICE("xm_load_xm0104_instrument", "ignoring non-zero instrument type %d", type);
    }

    /* Prescan already checked MAX_SAMPLES_PER_INSTRUMENT */
    num_samples = READ_U8(offset + 27);
    if(num_samples == 0) {
        #if HAS_FEATURE(FEATURE_MULTISAMPLE_INSTRUMENTS)
        memset(instr->sample_of_notes, 0xFF, sizeof(instr->sample_of_notes));
        #else
    ...
    vol_env_flags = READ_U8(offset + 233);
    xm_check_and_fix_envelope(&(instr->volume_envelope), vol_env_flags);
    #endif

    #if HAS_PANNING && HAS_FEATURE(FEATURE_PANNING_ENVELOPES)
    xm_load_xm0104_envelope_points(&instr->panning_envelope, moddata + offset + 177);
    ...
    pan_env_flags = READ_U8(offset + 234);
    ...
    /* Read sample headers */
    samples_index = ctx->module.num_samples;
    extra_samples_size = 0;
        ...
        if(HAS_FEATURE(FEATURE_MULTISAMPLE_INSTRUMENTS) || i == 0) {
            ...
            if(is_16bit) {
                /* Find some free bit in the struct to pack the
                   16bitness */
                _XM_STATIC_ASSERT_FUNCTION(MAX_SAMPLE_LENGTH < (1u << 31),"");
    ...
    for(i = 0; i < num_samples; ++i) {
        ...
        if(s->length & (1u << 31)) {
            ...
            #if _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int8_t 
                NOTICE("xm_load_xm0104_instrument", "16 bit sample will be dithered to 8 bits");
            #endif
...
static void xm_load_xm0104_envelope_points(xm_envelope_t* env,
    ...
    for(i = 0; i < MAX_ENVELOPE_POINTS; ++i) {
        ...
        if(env_val > MAX_ENVELOPE_VALUE) {
            NOTICE("xm_load_xm0104_envelope_points", "clamped invalid envelope pt value (%u -> %u)", env_val, MAX_ENVELOPE_VALUE);
...
static void xm_check_and_fix_envelope(xm_envelope_t* env, uint8_t flags) {
    ...
    if(env->num_points > MAX_ENVELOPE_POINTS) {
        NOTICE("xm_check_and_fix_envelope", "clamped invalid envelope num_points (%u -> %u)", env->num_points, MAX_ENVELOPE_POINTS);
    ...
    if(env->num_points < 2) {
        NOTICE("xm_check_and_fix_envelope", "discarding invalid envelope data (needs 2 point at least, got %u)", env->num_points);
        goto kill_envelope;
    }
    for(i = 1; i < env->num_points; ++i) {
        if(env->points[i-1].frame < env->points[i].frame) continue;
        NOTICE("xm_check_and_fix_envelope", "discarding invalid envelope data (point %u frame %X -> point %u frame %X)",
    ...
    if(env->loop_start_point >= env->num_points) {
        NOTICE("xm_check_and_fix_envelope", "clearing invalid envelope loop (start point %u > %u)",
               env->loop_start_point, env->num_points - 1);
        env->loop_start_point = 0;
        env->loop_end_point = 0;
    }
    if(env->loop_end_point >= env->num_points || env->loop_end_point < env->loop_start_point) {
        NOTICE("xm_check_and_fix_envelope", "clearing invalid envelope loop (end point %u, > %u or < %u)",
               env->loop_end_point, env->num_points - 1,
               env->loop_start_point);
        env->loop_start_point = 0;
        env->loop_end_point = 0;
    }
    if(env->loop_start_point == env->loop_end_point || !(flags & ENVELOPE_FLAG_LOOP)) {
        env->loop_start_point = 0;
        env->loop_end_point = 0;
    }

    if(env->sustain_point >= env->num_points) {
        NOTICE("xm_check_and_fix_envelope", "clearing invalid envelope sustain point (%u > %u)", env->sustain_point, env->num_points - 1);
 ...
 kill_envelope:
    ...
    memset(env, 0, sizeof(xm_envelope_t));
}
...
static uint32_t xm_load_xm0104_sample_header(xm_sample_t* sample, bool* is_16bit,
    ...
    if(loop_start > sample->length) {
        NOTICE("xm_load_xm0104_sample_header", "fixing invalid sample loop start");
        loop_start = sample->length;
    }
    if(loop_start + sample->loop_length > sample->length) {
        NOTICE("xm_load_xm0104_sample_header", "fixing invalid sample loop length");
    ...
    volume = READ_U8(offset + 12);
    if(volume > MAX_VOLUME) {
        NOTICE("xm_load_xm0104_sample_header", "clamping invalid sample volume (%u > %u)",
               volume, MAX_VOLUME);
        volume = MAX_VOLUME;
    }
    /* Assigning to bitfields is ugly with -Wconversion, it is how it is */
    _XM_STATIC_ASSERT_FUNCTION(MAX_VOLUME <= 0x7F,"");
    ...
    if(flags & ~(SAMPLE_FLAG_PING_PONG | SAMPLE_FLAG_FORWARD | SAMPLE_FLAG_16B)) {
        NOTICE("xm_load_xm0104_sample_header", "ignoring unknown flags (%d) in sample", flags);
    }
    ...
    #if XM_STRINGS
    _XM_STATIC_ASSERT_FUNCTION(SAMPLE_NAME_LENGTH >= 23,""); /* +1 for NUL */
...
static void xm_load_xm0104(xm_context_t* ctx, const char* moddata, uint32_t moddata_length) {
    ...
    for(i = 0; i < ctx->module.num_patterns; ++i) {
        offset = xm_load_xm0104_pattern(ctx, ctx->patterns + i, moddata, moddata_length, offset);
    }
    ...
    if(has_invalid_patterns) {
        ...
        for(i = 0; i < ctx->module.length; ++i) {
            ...
            ctx->module.pattern_table[i] = (uint8_t)ctx->module.num_patterns;
        }
        ctx->patterns[ctx->module.num_patterns].num_rows = EMPTY_PATTERN_NUM_ROWS;
        assert(ctx->module.num_rows < UINT16_MAX);
        ctx->patterns[ctx->module.num_patterns].rows_index = (uint16_t)ctx->module.num_rows;
    ...
    for(i = 0; i < num_instruments; ++i) {
        ...
        offset = xm_load_xm0104_instrument(ctx, inst, moddata, moddata_length, offset);
    }
}
...
static bool xm_prescan_mod(const char* moddata,
    ...
    min_sz = (uint32_t)(1084u + p->samples_data_length);
    if(moddata_length < min_sz) {
        NOTICE("xm_prescan_mod", "mod file too small, expected more bytes (%u < %u)", moddata_length, min_sz);
        return false;
    }
...
static void xm_load_mod(xm_context_t* ctx,
    ...
    #if HAS_FEATURE(FEATURE_LINEAR_FREQUENCIES) && HAS_FEATURE(FEATURE_AMIGA_FREQUENCIES)
    ...
    for(i = 0; i < ctx->module.num_samples; ++i) {
        ...
        #if HAS_FEATURE(FEATURE_SAMPLE_FINETUNES)
        finetune = READ_U8(offset + 24);
        if(finetune >= 16) {
            NOTICE("xm_load_mod", "ignoring invalid finetune of sample %u (%u)",
                   i+1, finetune);
            finetune = 8;
        }
        smp->finetune = (int8_t)((finetune < 8 ? finetune : finetune - 16) * 2);
        #endif

        volume = READ_U8(offset + 25);
        if(volume > MAX_VOLUME) {
            NOTICE("xm_load_mod", "clamping volume of sample %u (%u -> %u)",
        ...
        loop_start = (uint32_t)(READ_U16BE(offset + 26) * 2);
        loop_length = (uint32_t)(READ_U16BE(offset + 28) * 2);
        if(loop_length > 2) {
    ...
    if(ctx->module.length > 128) {
        NOTICE("xm_load_mod", "clamping module pot length %d to %d\n", ctx->module.length, 128);
        ctx->module.length = 128;
    }
    ...
    _XM_STATIC_ASSERT_FUNCTION(128 <= PATTERN_ORDER_TABLE_LENGTH,"");
    READ_MEMCPY(ctx->module.pattern_table, offset + 2, 128);
    offset += 134;

    #if HAS_PANNING && HAS_FEATURE(FEATURE_DEFAULT_CHANNEL_PANNINGS)
    /* Emulate hard panning (LRRL LRRL etc) */
    for(ch = 0; ch < p->num_channels; ++ch) {
        ctx->module.default_channel_panning[ch] = (((ch >> 1) ^ ch) & 1) ? 0xFF : 0x01;
    }
    #endif
    ...
    for(i = 0; i < ctx->module.num_patterns; ++i) {
        ...
        for(j = 0; j < NUM_CHANNELS(&ctx->module) * pat->num_rows; ++j) {
            ...
            if(period > 0) {
                ...
                assert(period < 112);
                for(i = 0; i < 11 && period < x_2[i]; ++i, ++slot->note);
            }
...
static void xm_load_mod_effect(xm_pattern_slot_t* slot) {
    ...
    if(slot->effect_type == 0xE) {
        ...
        case 0:
        case 0xF:
            NOTICE("xm_load_mod_effect", "removing unsupported E%02X",
                   slot->effect_param);
            slot->effect_type = 0;
            slot->effect_param = 0;
            return;
        ...
        case 7:
            slot->effect_type = EFFECT_SET_TREMOLO_CONTROL;
        fixup_control_param:
            new_param = WAVEFORM_SINE; /* just to remove the compiler warning */
    ...
    case 0xD:
        ...
        slot->effect_param = (uint8_t)(slot->effect_param - 6 * (slot->effect_param >> 4));
        return;
    case 0xF:
...
static bool xm_prescan_s3m(const char* moddata,
                           uint32_t moddata_length,
                           xm_prescan_data_t* out) {
    ...
    out->pot_length = 0;
    for(i = 0; i < pot_length; ++i) {
        x = READ_U8(96 + i);
        if(x == 255) break;
        if(x == 254) continue;
        if(out->pot_length == PATTERN_ORDER_TABLE_LENGTH) {
            NOTICE("xm_prescan_s3m", "order table too big");
            return false;
        }
        ++out->pot_length;
    }
    ...
    if(out->num_samples > MAX_INSTRUMENTS) {
        NOTICE("xm_prescan_s3m", "too many instruments (%u > %u)", out->num_instruments, MAX_INSTRUMENTS);
        return false;
    }

    out->num_patterns = READ_U16(36);
    if(out->num_patterns > MAX_PATTERNS) {
        NOTICE("xm_prescan_s3m", "too many patterns (%u > %u)", out->num_patterns, MAX_PATTERNS);
        return false;
    }
    ...
    out->num_channels = (uint8_t)_xm_count_ones(used_channels);
    ...
    for(i = 0; i < out->num_instruments; ++i) {
        ...
        length = READ_U32(ins_offset + 16);
        loop_start = READ_U32(ins_offset + 20);
        loop_end = READ_U32(ins_offset + 24);
        smp_flags = READ_U8(ins_offset + 31);
        if(smp_flags & 4) {
            length /= 2;
            loop_start /= 2;
            loop_end /= 2;
        ...
        if(length > MAX_SAMPLE_LENGTH) {
            NOTICE("xm_prescan_s3m", "sample %u too long (%u > %u)",
...
static void xm_load_s3m(xm_context_t* ctx,
                        const char* moddata,
                        uint32_t moddata_length,
                        const xm_prescan_data_t* p) {
    #if XM_STRINGS
    ...
    _XM_STATIC_ASSERT_FUNCTION(MODULE_NAME_LENGTH >= 28,"");
    READ_MEMCPY(ctx->module.name, 0, 27);
    tn = ctx->module.trackername;
    switch(tracker_version >> 12) {
    case 1:
        memcpy(tn, "Scream Tracker ", 15);
        tn += 15;
        goto version;

    case 2:
        memcpy(tn, "Imago Orpheus ", 14);
        tn += 14;
        goto version;

    case 3:
        memcpy(tn, "Impulse Tracker ", 16);
        tn += 16;
        goto version;

    case 4:
        memcpy(tn, "Schism Tracker", 14);
        break;
    ...
    for(ch = 0; ch < 32; ++ch) {
        ...
        if(channel_map[channel_settings[ch]] < 16) {
            NOTICE("xm_load_s3m", "multiple channels assigned to PCM%d, expect broken playback", channel_settings[ch]);
            continue;
        }
    ...
    if((READ_U8(51) & 128) == 0) {
        ...
        memset(ctx->module.default_channel_panning, MAX_PANNING / 2, num_channels);
...
void xm_load_s3m_instrument(xm_context_t* ctx,
    ...
    uint32_t base_length, base_loop_start, base_loop_end;
    uint8_t base_volume, base_flags;
    bool is_looped, is_stereo, is_16bit;

    uint32_t base_c_frequency;
    int16_t rel_finetune;
    ...
    #if XM_STRINGS
    _XM_STATIC_ASSERT_FUNCTION(INSTRUMENT_NAME_LENGTH >= 28,"");
    ...
    base_length = READ_U32(offset + 16);
    base_loop_start = READ_U32(offset + 20);
    base_loop_end = READ_U32(offset + 24);
    base_volume = READ_U8(offset + 28);
    base_flags = READ_U8(offset + 31);

    is_looped = base_flags & 1;
    is_stereo = base_flags & 2;
    is_16bit = base_flags & 4;

    if(is_stereo) {
        NOTICE("xm_load_s3m_instrument", "sample %x has stereo data, this is not supported", idx);
    }
    if(base_flags & 0xF8) {
        NOTICE("xm_load_s3m_instrument", "ignoring sample %x flags (%x)",
               idx, base_flags & 0xF8);
    }

    base_c_frequency = READ_U32(offset + 32);
    rel_finetune = (int16_t)(log2f((float)base_c_frequency / 8363.f) * 192.f);
    ...
    if(base_volume > MAX_VOLUME) {
        NOTICE("xm_load_s3m_instrument", "clamping volume of sample %u (%u -> %u)", idx, base_volume, MAX_VOLUME);
    ...
    offset = 16 * ((((uint32_t)READ_U8(offset + 13)) << 16) + READ_U16(offset + 14));
...
static void xm_load_s3m_pattern(xm_context_t* ctx,
                                uint8_t patidx,
                                const uint8_t* channel_settings,
    ...
    while(offset < stop && read_rows < 64) {
        ...
        s = slots + channel_map[channel_settings[x & 31]];
        ...
        if(x & 32) {
            ...
            if(s->note == 254) {
                ...
                s->note = (uint8_t)(1 + (s->note >> 4) * 12 + (s->note & 0xF));
            }
        }
        ...
        if(x & 128) {
            ...
            case 24:
                ...
                if(s->effect_param <= 0x80) {
                    s->effect_type = EFFECT_SET_CHANNEL_PANNING;
                    /* Remap from 0..=0x80 to 0..=0xFF */
                    s->effect_param = (s->effect_param == 0x80) ? 0xFF : (s->effect_param * 2);
                } else {
                    goto blank_effect;
                }
                break;
            ...
            case 19: /* XXX: effect memory logic is not a perfect
                        workaround */
                if(s->effect_param == 0) {
                    s->effect_param = last_effect_parameters[x & 31];
                    if(s->effect_param == 0) {
                        goto blank_effect;
                    } else {
                        NOTICE("xm_load_s3m_pattern", "conversion from S00 to S%02X in pattern %x might be inaccurate", s->effect_param, patidx);
                    }
                }
                ...
                fixup_control_param:
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        s->effect_param = WAVEFORM_RAMP_DOWN;
                        break;
                    case 2:
                        s->effect_param = WAVEFORM_SQUARE;
                        break;
                    case 3:
                        s->effect_param = WAVEFORM_RANDOM;
                        break;
                    }
                    break;
                ...
                case 0xA: /* XXX: contradicting info on SAy
                             (stereo control) */
                    s->effect_type = EFFECT_SET_CHANNEL_PANNING;
                    left_ch = channel_settings[x & 31] < 8;
                    switch(s->effect_param & 0xF) {
                    case 0:
                    case 2:
                        /* Normal panning */
                        s->effect_param = left_ch ? 0 : 0xFF;
                        break;
                    case 1:
                    case 3:
                        /* Reversed panning */
                        s->effect_param = left_ch ? 0xFF : 0;
                        break;
                    case 4:
                    case 5:
                    case 6:
                ...
                _XM_STATIC_ASSERT_FUNCTION(EFFECT_CUT_NOTE == (32|0xC), "");
                _XM_STATIC_ASSERT_FUNCTION(EFFECT_DELAY_NOTE == (32|0xD), "");
                _XM_STATIC_ASSERT_FUNCTION(EFFECT_DELAY_PATTERN == (32|0xE), "");
                case 0xC:
                case 0xD:
                case 0xE:
    ...
    if(offset != stop || read_rows != 64) {
        NOTICE("xm_load_s3m_pattern", "dodgey pattern %X has incorrect packed size", patidx);
    }
    ...
    for(i = 0; i < 64; ++i) {
        ...
        if(ch == NUM_CHANNELS(&ctx->module)) {
            NOTICE("xm_load_s3m_pattern", "inaccurate loop fixup in pattern %x (no space left)", patidx);
        }
    }
}
