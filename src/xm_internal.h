...
#include "old_c_compat.h"
/*  assert.h, math.h, stdbit.h and stdckdint.h are included with the header below 
    if available. Otherwise, there are fallbacks. May not be the best or fastest 
    fallbacks as these are my naive attempts. Uses whatever compiler extensions 
    available as needed. Check it out if you wanna disable static_asserts */



/*  Helper macros for preprocessor type checking. 
    Make sure to use a non-zero value             */
#define _XM_type_float 100000
#define _XM_type_int8_t 200000
#define _XM_type_int16_t 300000

#define _XM_typeof_(x) _XM_type_##x
#define _XM_typeof(x)  _XM_typeof_(x)



#define _XM_FILE_NAME xm_internal_h

/* start of actual libxm code below. Everything above was my crappy attempt
  at making this work for older build systems, e.g. VC 6.0 */
...
#define NOTICE xm_notice
#define TRACE xm_trace

#else
/* I am hoping the compilers are smart enough and would recognise (void)(arg1, arg2,... argN) as 
   (void)argN and discards accordingly. It should do it, I don't use side-effects with NOTICE    */
...
_XM_STATIC_ASSERT_GLOBAL(_XM_typeof(XM_SAMPLE_TYPE) == _XM_type_float || 
    _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int8_t || 
    _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int16_t, 
    "Unsupported value of XM_SAMPLE_TYPE");
_XM_STATIC_ASSERT_GLOBAL(XM_LOOPING_TYPE >= 0 && XM_LOOPING_TYPE <= 2, "Invalid value of XM_LOOPING_TYPE");
_XM_STATIC_ASSERT_GLOBAL(XM_LOOPING_TYPE != 1 || !HAS_EFFECT(0xB), "XM_LOOPING_TYPE=1 requires disabling Bxx (jump to order) effect");
_XM_STATIC_ASSERT_GLOBAL(XM_SAMPLE_RATE >= 0 && XM_SAMPLE_RATE <= UINT16_MAX, "Unsupported value of XM_SAMPLE_RATE");
...
_XM_STATIC_ASSERT_GLOBAL(HAS_FEATURE(FEATURE_LINEAR_FREQUENCIES) || HAS_FEATURE(FEATURE_AMIGA_FREQUENCIES), "Must enable at least one frequency type (linear or Amiga)");
...
#define MAX_SAMPLE_LENGTH (UINT32_MAX/SAMPLE_MICROSTEPS)
    ...
    #define SAMPLE_DATA(ctx, idx) ((float)(ctx)->samples_data[idx] / 32768.f)
#elif _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_int8_t 
    #define SAMPLE_DATA(ctx, idx) ((float)(ctx)->samples_data[idx] / 128.f)
#elif _XM_typeof(XM_SAMPLE_TYPE) == _XM_type_float
    #define SAMPLE_DATA(ctx, idx) ((ctx)->samples_data[idx])
#endif
...
struct xm_envelope_point_s {
    uint16_t frame;
    _XM_STATIC_ASSERT_STRUCT(MAX_ENVELOPE_VALUE < UINT8_MAX,"")
...
struct xm_envelope_s {
    ...
    _XM_STATIC_ASSERT_STRUCT(MAX_ENVELOPE_POINTS + 128 < UINT8_MAX,"")
...
struct xm_sample_s {
    ...
    bool ping_pong: 1;
    _XM_STATIC_ASSERT_STRUCT(MAX_VOLUME < (1<<7),"")
    ...
    #if XM_STRINGS
    _XM_STATIC_ASSERT_STRUCT(SAMPLE_NAME_LENGTH % 8 == 0,"")
...
struct xm_instrument_s {
    ...
    _XM_STATIC_ASSERT_STRUCT(MAX_NOTE % 2 == 0,"")
    ...
    #if XM_STRINGS
    _XM_STATIC_ASSERT_STRUCT(INSTRUMENT_NAME_LENGTH % 8 == 0,"")
...
struct xm_pattern_s {
    ...
    _XM_STATIC_ASSERT_STRUCT((MAX_PATTERNS - 1) * MAX_ROWS_PER_PATTERN < UINT16_MAX,"")
    uint16_t rows_index;
    uint16_t num_rows;
};
...
struct xm_module_s {
    ...
    _XM_STATIC_ASSERT_STRUCT(PATTERN_ORDER_TABLE_LENGTH % 8 == 0,"")
    ...
    #if XM_STRINGS
    _XM_STATIC_ASSERT_STRUCT(MODULE_NAME_LENGTH % 8 == 0,"")
    _XM_STATIC_ASSERT_STRUCT(TRACKER_NAME_LENGTH % 8 == 0,"")
...
struct xm_channel_context_s {
    ...
    _XM_STATIC_ASSERT_STRUCT(RAMPING_POINTS % 2 == 1,"")
...
uint16_t xm_rand16(uint32_t*);
void xm_tick(xm_context_t*);
void xm_print_pattern(xm_context_t*, uint8_t);
